...
//						Obsevation2.setSpacingBefore(5f); // Space before table
						...
						for (SummaryObservation supplyObs : summaryObser) {
							...
							if (supplyObs.getObservationComponentDetails().contains("alternate")&&(supplyObs.getObervationStatus()!=null&&!supplyObs.getObervationStatus().equalsIgnoreCase("R"))) {
...
//							InnInspOBS.setSpacingBefore(5f); // Space before table
							...
							float[] obsInspColumnWidths = { 40F, 40F, 40F, 40F };
							PdfPTable obsInsp = new PdfPTable(obsInspColumnWidths);
							...
							if (inspectionObs.getObservationComponentDetails()
									.equalsIgnoreCase("inspectionComponent")&&(inspectionObs.getObervationStatus()!=null&&!inspectionObs.getObervationStatus().equalsIgnoreCase("R"))) {
								Paragraph Location = new Paragraph("Location -" + count, font5);
...
//							if (inspectionObs.getObservationComponentDetails().equalsIgnoreCase("consumer-UnitIter")) {
                            if(inspectionObs.getMainsObservation()!=null) {
							List<MainsObservation> inspectionMainsObservation = inspectionObs.getMainsObservation();
							int i1 = 1;
							for (MainsObservation inspectionConsumerObservation : inspectionMainsObservation) {
                              if((inspectionConsumerObservation.getObervationStatus()!=null&&!inspectionConsumerObservation.getObervationStatus().equalsIgnoreCase("R"))&&inspectionConsumerObservation.getObservations()!=null&&!inspectionConsumerObservation.getObservations().isEmpty()) {
								PdfPCell mainsOb = new PdfPCell(new Paragraph("Mains Observations - " + i1, font9));
								mainsOb.setHorizontalAlignment(Element.ALIGN_CENTER);
//								mainsOb.setGrayFill(0.92f);
								obsInsp.addCell(mainsOb);

								PdfPCell mainsOb1 = new PdfPCell(
										new Paragraph(inspectionConsumerObservation.getObservations(), font9));
								mainsOb1.setHorizontalAlignment(Element.ALIGN_JUSTIFIED);
								obsInsp.addCell(mainsOb1);

								PdfPCell mainsOb2 = new PdfPCell(
										new Paragraph(inspectionConsumerObservation.getFurtherActions(), font9));
								mainsOb2.setHorizontalAlignment(Element.ALIGN_JUSTIFIED);
								obsInsp.addCell(mainsOb2);

								PdfPCell mainsOb3 = new PdfPCell(
										new Paragraph(inspectionConsumerObservation.getComment(), font9));
								mainsOb3.setHorizontalAlignment(Element.ALIGN_JUSTIFIED);
								obsInsp.addCell(mainsOb3);
...
//								}
                            ...
                            if(inspectionObs.getDbParentObservation()!=null) {
							List<DbParentObservation> inspectionDbParentObservation = inspectionObs
									.getDbParentObservation();
							int i2 = 1;
							int i2a = 1;
							for (DbParentObservation dbParentObservation : inspectionDbParentObservation) {
                               if(dbParentObservation.getDbParentStatus()!=null&&!dbParentObservation.getDbParentStatus().equalsIgnoreCase("R")&&childObservation(dbParentObservation.getDbChildObservation())) {
								PdfPCell distributionOb = new PdfPCell(
										new Paragraph("Distribution Board- " + i2, font5));
								distributionOb.setHorizontalAlignment(Element.ALIGN_CENTER);
								distributionOb.setGrayFill(0.92f);
								distributionOb.setColspan(4);
								obsInsp.addCell(distributionOb);

								for (DbChildObservation dbChildObservation : dbParentObservation
										.getDbChildObservation()) {
									if((dbChildObservation.getObervationStatus()!=null&&!dbChildObservation.getObervationStatus().equalsIgnoreCase("R"))&&dbChildObservation.getObservations()!=null&&!dbChildObservation.getObservations().isEmpty()) {
									PdfPCell dbChildOb = new PdfPCell(new Paragraph("" + i2a, font9));
									dbChildOb.setHorizontalAlignment(Element.ALIGN_CENTER);
//                            	dbChildOb.setGrayFill(0.92f);
									obsInsp.addCell(dbChildOb);

									PdfPCell dbChildOb1 = new PdfPCell(
											new Paragraph(dbChildObservation.getObservations(), font9));
									dbChildOb1.setHorizontalAlignment(Element.ALIGN_JUSTIFIED);
									obsInsp.addCell(dbChildOb1);

									PdfPCell dbChildOb2 = new PdfPCell(
											new Paragraph(dbChildObservation.getFurtherActions(), font9));
									dbChildOb2.setHorizontalAlignment(Element.ALIGN_JUSTIFIED);
									obsInsp.addCell(dbChildOb2);

									PdfPCell dbChildOb3 = new PdfPCell(
											new Paragraph(dbChildObservation.getComment(), font9));
									dbChildOb3.setHorizontalAlignment(Element.ALIGN_JUSTIFIED);
									obsInsp.addCell(dbChildOb3);
                            ...
                            if(inspectionObs.getSubDbObservation()!=null) {
							List<SubDbObservation> inspectionsubDbObservation = inspectionObs.getSubDbObservation();
							int i3 = 1;
							int i3a = 1;
							for (SubDbObservation subDbObservation : inspectionsubDbObservation) {
                              if(subDbObservation.getSubDbObservationStatus()!=null&&!subDbObservation.getSubDbObservationStatus().equalsIgnoreCase("R")&&subDbchildObservation(subDbObservation.getSubDbChildObservation())) {
								PdfPCell subDbOb = new PdfPCell(new Paragraph("Sub Distribution Board- " + i3, font5));
								subDbOb.setHorizontalAlignment(Element.ALIGN_CENTER);
								subDbOb.setGrayFill(0.92f);
								subDbOb.setColspan(4);
								obsInsp.addCell(subDbOb);

								for (SubDbChildObservation subdbChildObservation : subDbObservation
										.getSubDbChildObservation()) {
									if((subdbChildObservation.getObervationStatus()!=null&&!subdbChildObservation.getObervationStatus().equalsIgnoreCase("R"))&&subdbChildObservation.getObservations()!=null&&!subdbChildObservation.getObservations().isEmpty()) {
									PdfPCell subDbChildOb = new PdfPCell(new Paragraph("" + i3a, font9));
									subDbChildOb.setHorizontalAlignment(Element.ALIGN_CENTER);
//                            	subDbChildOb.setGrayFill(0.92f);
									obsInsp.addCell(subDbChildOb);

									PdfPCell subDbChildOb1 = new PdfPCell(
											new Paragraph(subdbChildObservation.getObservations(), font9));
									subDbChildOb1.setHorizontalAlignment(Element.ALIGN_JUSTIFIED);
									obsInsp.addCell(subDbChildOb1);

									PdfPCell subDbChildOb2 = new PdfPCell(
											new Paragraph(subdbChildObservation.getFurtherActions(), font9));
									subDbChildOb2.setHorizontalAlignment(Element.ALIGN_JUSTIFIED);
									obsInsp.addCell(subDbChildOb2);

									PdfPCell subDbChildOb3 = new PdfPCell(
											new Paragraph(subdbChildObservation.getComment(), font9));
									subDbChildOb3.setHorizontalAlignment(Element.ALIGN_JUSTIFIED);
									obsInsp.addCell(subDbChildOb3);
...
//						Testing Observation Start from here
						...
						int i1 = 1;
						List<SummaryObservation> strList = new ArrayList<>();
						List<SummaryObservation> locationList = new ArrayList<>();

						strList = summaryObser
								.stream().filter(summaryObservation -> summaryObservation
										.getObservationComponentDetails().contains("circuit")&&!summaryObservation.getObervationStatus().equalsIgnoreCase("R"))
								.collect(Collectors.toList());
						String observationComponent = "";
						for (SummaryObservation strList1 : strList) {
							String observationComponentDetails[] = strList1.getObservationComponentDetails().split(",");
							if (!observationComponentDetails[1].equals(observationComponent)) {
								observationComponent = observationComponentDetails[1];
								locationList.add(strList1);
							}
						}
						Integer location=1;
						for(SummaryObservation locationList1:locationList) {
							...
							PdfPTable InnTestingOBS1 = new PdfPTable(pointColumnWidths);
							InnTestingOBS1.setWidthPercentage(100); // Width 100%
//							InnTestingOBS1.setSpacingBefore(5f); // Space before table
							InnTestingOBS1.getDefaultCell().setBorder(0);
							float[] obsTestingColumnWidths = { 40F, 40F, 40F, 40F };
							PdfPTable obsTest = new PdfPTable(obsTestingColumnWidths);
							obsTest.setWidthPercentage(100); // Width 100%
							
							Paragraph Location = new Paragraph("Location -" + location, font5);
							Location.setAlignment(Element.ALIGN_LEFT);
							document.add(Location);
							observationHead(document);
							String observationComponentDetails[] = locationList1.getObservationComponentDetails().split(",");
							String currentDetailsTesting="";
							Integer detailsTestingNumber=1;
						    for (SummaryObservation testingObs : strList) {
								String observationComponentDetailsInner[] = testingObs.getObservationComponentDetails().split(",");
								if (observationComponentDetails[1].equalsIgnoreCase(observationComponentDetailsInner[1])) {
									if(!observationComponentDetailsInner[2].equalsIgnoreCase(currentDetailsTesting)) {
										detailsTestingNumber=1;
										currentDetailsTesting=observationComponentDetailsInner[2];
										PdfPCell detailsTesting = new PdfPCell(
												new Paragraph(observationComponentDetailsInner[2], font5));
										detailsTesting.setHorizontalAlignment(Element.ALIGN_CENTER);
										detailsTesting.setGrayFill(0.92f);
										detailsTesting.setColspan(4);
										obsTest.addCell(detailsTesting);
									}
									
									PdfPCell testOb4 = new PdfPCell(
											new Paragraph("Observation - " + detailsTestingNumber, font9));
...
//				for (SummaryDeclaration declaration : declaration1) {
	...
	private boolean subDbchildObservation(List<SubDbChildObservation> subDbChildObservation) {
		 for(SubDbChildObservation subDbChildObservation1:subDbChildObservation) {
	        	if((subDbChildObservation1.getObervationStatus()!=null&&!subDbChildObservation1.getObervationStatus().equalsIgnoreCase("R"))&&subDbChildObservation1.getObservations()!=null&&!subDbChildObservation1.getObservations().isEmpty()) {
	        		return true;
	        	}
	        }
		return false;
	...
	private boolean childObservation(List<DbChildObservation> dbChildObservation) {
        for(DbChildObservation dbChildObservation1:dbChildObservation) {
        	if((dbChildObservation1.getObervationStatus()!=null&&!dbChildObservation1.getObervationStatus().equalsIgnoreCase("R"))&&dbChildObservation1.getObservations()!=null&&!dbChildObservation1.getObservations().isEmpty()) {
        		return true;
        	}
        }
		return false;

...
//				document.add(paragraph10);
				...
				for (SignatorDetails arr : convertion) {
					if (!arr.getSignatorStatus().equalsIgnoreCase("R") && arr.getSignatorRole().equalsIgnoreCase("designer1")) {
						declarationDesigner(document, arr);
					} else if (!arr.getSignatorStatus().equalsIgnoreCase("R") && arr.getSignatorRole().equalsIgnoreCase("designer2")) {
						Paragraph paragraph13 = new Paragraph("Designer: 2", font10N);
						document.add(paragraph13);
						if (!arr.getPersonName().equalsIgnoreCase("Unknown")) {
...
//		cell33.setFixedHeight(30f);
	...
	private void addRow1(PdfPTable table, String string1, byte[] imageByte, String string3, String string4,
		...
		Image image = null;
		PdfPCell valueCell1 = null;
		if (imageByte !=null) {
			image = Image.getInstance(imageByte);
			image.setAbsolutePosition(0, 0);
			image.scaleToFit(30, 50);
			 valueCell1 = new PdfPCell( image);
		}
		else {
			 valueCell1 = new PdfPCell(new Paragraph("", new Font(BaseFont.createFont(), 10, Font.NORMAL)));
		}
